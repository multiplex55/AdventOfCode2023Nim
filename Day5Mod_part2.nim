...
proc processRange(startSeed: int, rangeLength: int, maps: AllMaps): int {.thread.} =
  result = high(int)
  for seed in countup(startSeed, startSeed + rangeLength - 1):
    result = min(result, applyMapping(seed, maps))

proc processSeedRangesParallel(seedRanges: seq[tuple[start: int, length: int]], maps: AllMaps): int =
