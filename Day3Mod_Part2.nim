...
proc WalkAndFindNumbers(engineSchematic: seq[seq[string]], originalRowIndex: int, originalColIndex: int) : MergedCell = 
    var
        cells: seq[Cell]
    ...
    let mc: MergedCell = (parseInt(mergedNumString),mergedRowString.parseInt(),mergedColString.parseInt() )

    return mc

proc CheckForNumbers(engineSchematic: seq[seq[string]], originalRowIndex: int,originalColIndex: int) : seq[MergedCell] = 
    var
    ...
    if originalRowIndex - 1 >= 0:
        ...
        if characterAtIndex.isDigit():
            ...
            mergedCells.add(WalkAndFindNumbers(engineSchematic, originalRowIndex-1,originalColIndex))
    ...
    if originalColIndex + 1 <= maxCol:
        ...
        if characterAtIndex.isDigit():
            ...
            mergedCells.add(WalkAndFindNumbers(engineSchematic, originalRowIndex,originalColIndex+1))
            #echo &"Current mergedcells is"
            #for mc in mergedCells:
                #echo &"{mc}"
            #echo "================="
    ...
    if originalRowIndex - 1 >= 0 and originalColIndex + 1 <= maxCol:
        ...
        if characterAtIndex.isDigit():
            ...
            mergedCells.add(WalkAndFindNumbers(engineSchematic, originalRowIndex-1,originalColIndex+1))
        ...
        for currentColCounter,colV in engineSchematic[currentRowCounter]:
            ...
            if engineSchematic[currentRowCounter][currentColCounter] != ".":
                if not engineSchematic[currentRowCounter][currentColCounter][0].isDigit:
                    if engineSchematic[currentRowCounter][currentColCounter][0] == '*':
                        #This is not a dot or digit
                        var currentSeq = CheckForNumbers(engineSchematic, currentRowCounter, currentColCounter)
                        #echo "$$$$$$$$$$$$$$$$$$$$$$$$ - " & $currentSeq.len()
                        if currentSeq.len() == 2:
                            var temp = CheckForNumbers(engineSchematic, currentRowCounter, currentColCounter)
